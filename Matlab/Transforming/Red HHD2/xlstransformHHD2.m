function   xlstransformGR(Files,folder)
...
%%
    ...
    %For Reporter Assays
    if contains(ID1,'HHD2') && contains(ID2,'St2')
    ...
    %For the measurements that do not contain kinetics, just do the median
        ...
        %Red is the second pack of measurements !!!!!!!!!!!!!CHANGED FROM
        %THE HHD version
        if length(PControlValue)>3
            PControlValue1=median(PControlValue(1:end));
            RedControlValue1=median(RedControlValue(1:(end)));
            data1=median(data(:,1:(end)),2,'omitnan');
        end
    %%
    %Remove C- and R+ columns and determine the column where the R+ control
    %will be.
    ...
    %%
    %Writing everything inside
    xlswrite(write,[RedControlValue1,PControlValue1],1,strcat(wrpos,'3'));
    xlswrite(write,cellstr({'R+','Quenched'}),1,strcat(wrpos,'1'));
    xlswrite(write,date,1,'A1');
    xlswrite(write,data1',1,'B3')
    xlswrite(write,wells',1,'B1');
    xlswrite(write,sample',1,'B2');
end
end