function HHD2module(Files,folder,foldrep)
    ...
    [time,~]=xlsread(file2,2,'A6:A1000'); %Time of the experiments
    ...
    for hhh=1:size(data,2)
        ...
        kl=zeros(1,3);
            ...
            t90=time(t90ind);
            exper=data(5:end,hhh);
            Mab=data(2,hhh);Temp=data(3,hhh);Report=data(4,hhh);
   ...
   %% Initial time estimation
            ...
            %k(3): Monomer (M5) amount
                ...
                %k=[10^((mmm*2)-3),testim,data(2,mmm)];
                k=[10^8,-0.775455426,4.080988558];
                fittingfun = @(k) FittingHHD2(k,time,exper,Mab,Report,Temp,t90ind);
            ...
            [~,hcl]=(min(MSI));  %Select the fitting with less error
            MSE=MSI(hcl);
            k=kl(hcl,:);


            
            sol(1:2,hhh)=[k(1),k(3)]; %Fitted k,initial time and concentration Mab
            sol(3,hhh)=0.042;
            sol(5,hhh)=Temp;
            sol(6,hhh)=Report;
            sol(4,hhh)=k(2);
            ...
            %%%%%%%%%%%%%%% PLOTTING PART %%%%%%%%%%%%%%%%%%%%
            [Err,Sv5,tb]=PlotHHD2(sol(:,hhh),time,exper);
            ...
            xlabel('Time (mins)') 
            subplot(2,1,2),plot (time2,(exper-Err)./Err);
