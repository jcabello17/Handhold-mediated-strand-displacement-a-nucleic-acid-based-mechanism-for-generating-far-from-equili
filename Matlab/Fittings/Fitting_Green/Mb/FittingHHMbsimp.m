function output = FittingHHMbsimp(k,krep,k5,t,data,a,fb,b,c,t90,kbind,std)
...
%Constrains
if  (sum(k(1:4) < 0) > 0)  || k(5) >= 0 || k(3)<kbind-std || k(3)>kbind+std %Boundaries obtained from STD of 20hh kbind.
    output = NaN;    % give a NaN to the result if the boundaries are not respected
    return;                % Respect!!!
end
...
%%%%%% PREALLOCATING MATRICES  %%%%%%%%%
        ...
        if t90==0
            if k(4)<a*0.9 || k(4)>a*1.1    
                output = NaN;
                return;
            end          
        end
        
        tf=t-k(5); %Modify the time matrix
        tf=[0;tf]; %patching....
        x0=[0,0,0,0,fb,0,0]; %Initial conditions
        
        opts = odeset('Jacobian',@(t,x)J(tf,x,k,k(3),krep,k5,k(4),b,fb,c)); %Options to introduce the Jacobian 
        [~,Sv] = ode23s(@(t,x)Mb(tf,x,k,k(3),krep,k5,k(4),b,fb,c), tf , x0, opts);
        Sv(1,:)=[];%Patching to avoid initial conditions....
        
        if size(Sv(:,4),1)<size(data,1)
            output=NaN; %if the solver suddenly stops, restart the function
            return;
        end
    ...
    output = log10(sum(((data-result).^2)./(result+realmin)));
