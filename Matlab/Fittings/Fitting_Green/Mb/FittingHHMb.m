function y = FittingHHMb(k,std,kbind,krep,k5,t,data,a,fb,b,c,kdetach,t90,round)
...
%Constrains
if  (sum(k(1:3+size(data,2)) < 0) > 0)  || (sum(k(3+size(data,2)+1:3+2*size(data,2)) >= 0) > 0)  || k(3)>kbind+std || k(3)<kbind-std  
    y = NaN;    % give a NaN to the result if the boundaries are not respected
    return;                % Respect!!!
end

if round==1||round==2
    if k(2)<(kdetach/sqrt(10)) || k(2)>(kdetach*sqrt(10))%Allow kdetach to fluctuate an order of magnitude.
        y=NaN;
        return
    end
end
%%%%%% PREALLOCATING MATRICES  %%%%%%%%%
    ...
    for mmm=1:size(data,2)        
        if t90(mmm)==0
            if k(3+mmm)<a(mmm)*0.9 || k(3+mmm)>a(mmm)*1.1    
                y = NaN;
                return;
            end          
        end
        
        tf=t-k(3+size(data,2)+mmm); %Modify the time matrix
        tf=[0;tf]; %patching....
        x0=[0;0;0;0;fb(mmm);0;0]; %Initial conditions
        
        opts = odeset('Jacobian',@(t,x)J(tf,x,k,k(3),krep,k5,k(3+mmm),b(mmm),fb(mmm),c(mmm))); %Options to introduce the Jacobian 
        [~,Sv] = ode23s(@(t,x)Mb(tf,x,k,k(3),krep,k5,k(3+mmm),b(mmm),fb(mmm),c(mmm)), tf , x0, opts);
        ...
        result(:,mmm)=Sv(:,5)+Sv(:,6)+Sv(:,7)-fb(mmm); %make a matrix with 4 results 
    
    end
    result(find(result<0))=0.001;
