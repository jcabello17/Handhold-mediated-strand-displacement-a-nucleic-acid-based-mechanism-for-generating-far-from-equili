...
%% Parameter to change
ind=0; %0 for fitting in groups of 4; 1 for individual fitting
%%
...
for jjj=1:size(Files,1)
    ...
    if ind==0
            HHMbmodule(Files(jjj),folder,foldrep,foldM5); 
    elseif ind==1
            HHMbindividualfit(Files(jjj),folder,foldrep,foldM5);
    end
end
...
elseif ind==1
    mkdir(folder,'PlotsInd') % Plots folder
end
...
for iFig = 1:length(FigList)
  ...
  elseif ind==1
    savefig(FigHandle, strcat(folder,'\PlotsInd\',FigName));
  end
  close(FigHandle);
end
